...
## Account connections & daily sync

1. Add the new integration and notification environment variables to `.env.local`:
   - `PLAID_CLIENT_ID`, `PLAID_SECRET`, `PLAID_ENV`, and optional `PLAID_REDIRECT_URI` for Plaid Link.
   - `RESEND_API_KEY` / `RESEND_FROM_EMAIL` for email delivery (optional – falls back to console logging).
   - `INTERNAL_CRON_SECRET` to secure the background sync endpoint.
2. Schedule a Vercel Cron job (Project Settings → Cron Jobs) that issues a `POST` request to `/api/sync/daily` at your desired cadence. Set the `Authorization` header to `Bearer ${INTERNAL_CRON_SECRET}`.
3. Use the `/connections` dashboard view to launch Plaid Link, trigger manual refreshes, add manual assets, and review notifications generated by scheduled syncs.
